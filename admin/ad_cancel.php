...
<body>
    ...
    <header class="dashboard-header">
        <div class="header-container">
            ...
            <nav class="nav-links">
                ...
                <a href="ad_cancel.php" class="nav-link active">Cancelled</a>
    ...
    <main class="main-content">
        <h1 class="main-heading">Cancelled Requests</h1>
    ...
    <script>
        ...
        function getBadgeClass(status) {
            ...
            return statusMap[status.trim()] || 'badge-not-processed';
        }

        function getConfirmBadgeClass(status) {
            if (status === 'Approved') return 'badge-confirm-approved'; // Green
            if (status === 'Denied') return 'badge-confirm-denied';     // Red
        ...
        function fetchDataAndRenderTable(searchTerm = '') {
            ...
            let url = 'admin_cancelled_requests.php'; 
                ...
                .then(data => {
                    if (data.status === "success" && data.data && data.data.length > 0) {
                        ...
                        data.data.forEach(request => {
                            const dbProgress = request.progress || 'Not Processed';
                            const dbConfirmation = request.payment_confirmation || 'Pending';
                            const payStatus = request.payment_status || 'Unpaid';
                            ...
                            const rowClass = (finalStatus === 'Pending' && payStatus === 'Paid') ? 'row-new' : '';
                            ...
                            const row = `
                                <tr class="table-row ${rowClass}">
                                    ...
                                    <td class="table-cell" style="text-align:center;">
                                        <input type="date" value="${request.date_of_claiming || ''}" class="date-input" style="${dateStyle}" ${isDateDisabled}>
                                    </td>
