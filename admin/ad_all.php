...
<head>
    ...
    <title>All Requests Dashboard</title>
...
<body>
    ...
    <header class="dashboard-header">
        <div class="header-container">
            ...
            <nav class="nav-links">
                ...
                <a href="ad_all.php" class="nav-link active">ALL</a>
    ...
    <main class="main-content">
        <h1 class="main-heading">All Requests</h1>
    ...
    <script>
        ...
        function createStatusDropdown(currentStatus, type, reqId, isDisabled) {
            ...
            let html = `<select id="progress-select-${reqId}" class="status-select status-badge ${getBadgeClass(safeCurrent)}" 
            ...
            options.forEach(status => {
                const isSelected = status.toLowerCase() === safeCurrent.toLowerCase();
                if (isSelected) matchFound = true;
                
                const selectedAttr = isSelected ? 'selected' : '';
                html += `<option value="${status}" ${selectedAttr}>${status}</option>`;
            });

            if (!matchFound && safeCurrent !== '') {
        ...
        function validateProgressChange(selectElement) {
            ...
            const reqId = selectElement.id.replace('progress-select-', '');
            
            const paymentStatusEl = document.getElementById(`payment-status-${reqId}`);
            const paymentStatus = paymentStatusEl ? paymentStatusEl.innerText.trim() : '';

            // If Paid, cannot be changed to Cancelled
            if (paymentStatus === 'Paid' && newStatus === 'Cancelled') {
                alert("Action not allowed: You cannot cancel a request that has been Paid.");
                ...
                .then(data => {
                    if (data.status === "success" && data.data && data.data.length > 0) {
                        ...
                        data.data.forEach(request => {
                            ...
                            if (dbProgress.toLowerCase() === 'cancelled') {
                                finalStatus = 'Denied';
                            } else if (['Pending', 'In Progress', 'Ready for Claiming', 'Claimed'].includes(dbProgress)) {
                                finalStatus = 'Approved'; 
                            } else {
                                finalStatus = dbConfirmation;
                            }
                            
                            const isNotProcessed = (dbProgress === 'Not Processed' || dbProgress === 'Pending');
                            const rowClass = isNotProcessed ? 'row-new' : '';
        ...
        function updateRequest(reqId) {
            ...
            if (paymentVal === 'Paid' && progressVal === 'Cancelled') {
                alert("Action not allowed: You cannot cancel a request that has been Paid.");
