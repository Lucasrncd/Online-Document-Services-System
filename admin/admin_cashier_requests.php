...
$whereClauses = ["rst.`payment_status` = 'Paid'"]; 

if (isset($_GET['search']) && !empty($_GET['search'])) {
    $searchTerm = $conn->real_escape_string($_GET['search']);
    $safeSearchTerm = "%{$searchTerm}%";
    $whereClauses[] = "(rst.`req_id` LIKE '{$safeSearchTerm}' OR rst.`full-name` LIKE '{$safeSearchTerm}' OR pit.receipt_number LIKE '{$safeSearchTerm}')";
}
...
try {
    $sql_requests = "
        SELECT 
            ...
            rst.`payment_status`, rst.`progress`, 
            pit.`payment_confirmation`,
            pit.receipt_number, pit.proof_of_receipt, pit.date as date_paid
    ...
    while ($row = $result_requests->fetch_assoc()) {
        ...
        $requests[$req_id]['payment_info'] = [
            'proof_of_receipt' => $row['proof_of_receipt'],
    ...
    foreach ($final_data as &$req) {
        $req['documents_display'] = implode('<br>', $req['documents_requested']);
        $req['final_amount'] = number_format($req['final_amount'], 2);
    }
